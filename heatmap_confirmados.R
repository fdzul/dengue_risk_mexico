...
#' @param alpha_text is the font size of the text.
...
#'
#' @examples
heatmap_confirmados <- function(dataset, year, size_text, alpha_text,
    ...
    if(EDO == TRUE){
        ...
        plotly::ggplotly(ggplot2::ggplot(data = x, 
                             ...
                             ggplot2::geom_text(col = ifelse(x$n >= as.integer(quantile(x$n, probs = .75)), "white","gray"), 
                                                size = size_text, 
                                                alpha = alpha_text) +
                             #theme_minimal()+
            ...
            if(MPO == TRUE){
                ...
                x <- dataset |>
                    ...
                    dplyr::filter(DES_DIAG_FINAL %in% 
                                      c("DENGUE CON SIGNOS DE ALARMA", "DENGUE NO GRAVE", 
                                        "DENGUE GRAVE")) |>
                    dplyr::mutate(DES_MPO_RES = paste(CVE_EDO_RES, 
                                                      DES_MPO_RES, 
                                                      sep = " ")) |>
                    dplyr::group_by(DES_MPO_RES, SEM) |>
                    dplyr::summarise(n = dplyr::n(), .groups = "drop") |>
